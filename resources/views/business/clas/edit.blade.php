@extends('business.layout')
    @section('header')
        ...
        <title>Edit Classes | Ayo Sekolah</title>
    @endsection
    @section('contents')
    <div class="container pd-x-0 pd-lg-x-10 pd-xl-x-0">
            <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
                <div>
                <h4 class="mg-b-0 tx-spacing--1">Edit Classes</h4>
                </div>
            </div>
            <div class="row row-xs">
                <div class="col-sm-12 col-lg-12">
                    <div class="card card-body">
                        <form action="{{ route('business.clas.update',$clas->id) }}" method="POST" data-parsley-validate>
                        @csrf
                        <input name="_method" type="hidden" value="PATCH">
                        <div class="form-group">
                            <label for="formGroupExampleInput" class="d-block">Name</label>
                            <input type="text" class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="Enter your Class Name" name="name" value="{{ $clas->name }}" required>
                            @if ($errors->has('name'))
                        ...
                        <button class="btn btn-primary" type="submit"><i class="fas fa-check"></i> Update</button>
                        </form>
                    </div>
                </div><!-- col -->
            </div><!-- row -->
