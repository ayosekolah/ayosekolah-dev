<div class="card card-body">
    <form action="{{ url('personal/my_quiz/clas/update/'.$quiz->id) }}" method="POST"  enctype="multipart/form-data" data-parsley-validate>
        @csrf
    ...
    <div class="form-group">
            ...
            <input name="code" id="codeQuiz" onkeyup="getCode(this.value)" type="hidden" class="form-control" value="{{ $quiz->code }}" required>
            @if ($errors->has('code'))
            <ul class="parsley-errors-list filled" id="parsley-id-5">
    ...
    <div class="form-group">
        <label for="formGroupExampleInput" class="d-block">Busines : {{ $quiz->busines->name }}</label>
    </div>
